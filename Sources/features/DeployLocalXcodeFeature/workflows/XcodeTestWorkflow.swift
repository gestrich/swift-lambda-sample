...
public struct XcodeTestWorkflow: Sendable {
    ...
    private func runWorkflow(
        ...
        let isRunning = await service.isLambdaRunning()
        if !isRunning {
            continuation.yield(Progress(
                step: .checkingLambda,
                detail: .output("Lambda is not running, will start it")
            ))
            try await service.startLambda(output: nil)
            try await service.waitForReady()
        }
        continuation.yield(Progress(step: .checkingLambda, detail: .output("Lambda is running")))
